...
export async function GET(
  ...
  try {
    ...
    const fetchContent = request.nextUrl.searchParams.get('fetch_content') === 'true';
    ...
    const response = await fetch(`${backendUrl}/api/v1/assessment/datasets/${dataset_id}${queryString}`, {
      method: 'GET',
    ...
    if (fetchContent) {
      ...
      return NextResponse.json({ ...data, file_content: base64 }, { status: 200 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    const response = await fetch(`${backendUrl}/api/v1/assessment/datasets/${dataset_id}`, {
      method: 'DELETE',
      headers: {
        'X-API-KEY': apiKey,
      },
