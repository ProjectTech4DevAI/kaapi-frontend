...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[GET /api/guardrails/validators/configs] Fetching from:', url);
    ...
    console.log('[GET /api/guardrails/validators/configs] Backend response status:', response.status, response.statusText);

    // Handle empty responses (204 No Content, etc.)
    const text = await response.text();
    const data = text ? JSON.parse(text) : { success: true, data: [] };

    console.log('[GET /api/guardrails/validators/configs] Backend response data:', JSON.stringify(data, null, 2));

    // Return the response with the same status code
    if (!response.ok) {
      console.error('[GET /api/guardrails/validators/configs] Backend error:', response.status, data);
