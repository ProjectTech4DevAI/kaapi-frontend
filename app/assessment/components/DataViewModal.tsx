  ...
  return (
    ...
    >
      <div
        ...
        style={{ backgroundColor: colors.bg.primary, width: '85vw', maxWidth: '1100px', maxHeight: '80vh' }}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ backgroundColor: colors.bg.secondary, borderBottom: `1px solid ${colors.border}` }}>
                <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wide sticky top-0" style={{ color: colors.text.secondary, backgroundColor: colors.bg.secondary, width: '40px' }}></th>
            ...
            <tbody>
              {rows.map((row, rowIdx) => (
                <tr key={rowIdx} style={{ borderBottom: `1px solid ${colors.border}` }}>
                  <td className="px-4 py-2.5 text-xs" style={{ color: colors.text.secondary }}>{rowIdx + 1}</td>
...
export function jsonResultsToTableData(
  ...
  const skipFields = opts?.skipFields ?? new Set([
    'assessment_id', 'dataset_id', 'dataset_name', 'run_id', 'run_name',
    'run_status', 'config_id', 'config_version', 'response_id',
    'input_tokens', 'output_tokens', 'total_tokens', 'updated_at',
    'result_status', 'error', 'row_id', 'experiment_name',
  ]);
