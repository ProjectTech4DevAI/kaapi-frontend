...
export default function PromptEditorStep({
  ...
  const mentionOptions = useMemo(() => {
    ...
    return textColumns.filter((column) => column.toLowerCase().includes(normalized));
  ...
  const computeCaretPosition = useCallback(() => {
    ...
    return {
      top: markerRect.top - mirrorRect.top - textarea.scrollTop + parseInt(style.lineHeight, 10) + 6,
  ...
  const handleInput = useCallback(() => {
    ...
    let index = cursor - 1;
    while (index >= 0 && value[index] !== '@' && value[index] !== ' ' && value[index] !== '\n' && value[index] !== '\t') {
      index -= 1;
    }
  ...
  const insertMention = useCallback((column: string) => {
    const textarea = textareaRef.current;
  ...
  useEffect(() => {
    if (dropdownRef.current && mentionOptions.length > 0) {
      const active = dropdownRef.current.children[mentionIndex] as HTMLElement | undefined;
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      setEditorWidth(Math.max(MIN_EDITOR_WIDTH, Math.min(MAX_EDITOR_WIDTH, percent)));
    };
  ...
  const orderedColumns = useMemo(() => {
    ...
    const unused = textColumns.filter((column) => !usedColumns.includes(column));
    return [...used, ...unused];
  ...
  const previewText = useMemo(() => {
    ...
    textColumns.forEach((column) => {
      ...
      next = next.replace(new RegExp(`\\{${safeColumn}\\}`, 'g'), sampleRow[column] || '');
    });
    return next;
  ...
  return (
    <div className="mx-auto flex min-h-full max-w-6xl flex-col">
      <div className="flex-1 space-y-3 pb-20">
        ...
        <div className="rounded-xl px-4 py-3" style={{ backgroundColor: colors.bg.secondary }}>
            ...
            {orderedColumns.map((column) => {
              ...
              return (
                <button
                  ...
                  style={{
                    backgroundColor: isUsed ? 'rgba(22, 163, 74, 0.12)' : colors.bg.primary,
                    borderColor: isUsed ? 'rgba(22, 163, 74, 0.3)' : colors.border,
        ...
        >
          <div className="flex min-h-[320px] flex-col" style={{ width: `${editorWidth}%` }}>
                  ...
                  {mentionOptions.map((column, index) => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          backgroundColor: isActive ? colors.bg.secondary : colors.bg.primary,
                          color: colors.text.primary,
                        }}
          ...
          >
            <div className="h-full w-px" style={{ backgroundColor: colors.border }} />
          </div>
          ...
          <div className="min-h-[320px] flex-1">
            ...
            <div className="h-full lg:pl-4">
              ...
              >
                {!promptTemplate.trim() ? (
                  <span style={{ color: colors.text.secondary }}>Preview will appear here.</span>
                ) : Object.keys(sampleRow).length === 0 ? (
                  <span style={{ color: colors.text.secondary }}>
                    Sample data not available — go back to the Datasets tab, re-select your dataset, and return here.
                  </span>
