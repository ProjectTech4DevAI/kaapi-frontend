...
function AccordionSection({ title, isOpen, onToggle, onEdit, badge, children }: AccordionSectionProps) {
  return (
    <div
      className="border rounded-lg overflow-hidden"
      style={{ borderColor: isOpen ? colors.accent.primary : colors.border, backgroundColor: colors.bg.primary }}
    >
      <div
        onClick={onToggle}
        className="w-full flex items-center justify-between px-4 py-3.5 text-left transition-colors cursor-pointer"
        style={{ backgroundColor: isOpen ? 'rgba(59, 130, 246, 0.03)' : colors.bg.primary }}
        role="button"
        tabIndex={0}
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-semibold" style={{ color: colors.text.primary }}>{title}</span>
          {badge && (
            <span
        ...
        <button
          ...
          style={{ color: colors.accent.primary, backgroundColor: colors.bg.secondary }}
        >
          Edit
        </button>
      </div>
...
export default function ReviewStep({
  ...
  const { datasetId, columnMapping, promptTemplate, outputSchema, configs } = formState;
  ...
  return (
    <div className="mx-auto flex min-h-full max-w-2xl flex-col">
      ...
      <div className="mb-2">
        <h2 className="text-lg font-semibold" style={{ color: colors.text.primary }}>Review & Submit</h2>
      ...
      {/* Experiment Name */}
      <div className="border rounded-lg p-4" style={{ borderColor: colors.border, backgroundColor: colors.bg.primary }}>
        ...
        />
        <p className="text-[11px] mt-1.5" style={{ color: colors.text.secondary }}>
      ...
      >
        <div className="pt-2">
          <div className="text-xs font-medium mb-1" style={{ color: colors.text.secondary }}>Dataset ID</div>
          <div className="text-sm font-mono" style={{ color: colors.text.primary }}>{datasetId}</div>
        </div>
      </AccordionSection>
        ...
        <div className="space-y-3 pt-2">
          {columnMapping.textColumns.length > 0 && (
            <div className="flex items-start gap-3">
              <span className="text-xs font-medium w-20 flex-shrink-0 pt-0.5" style={{ color: colors.text.secondary }}>Text</span>
              <div className="flex flex-wrap gap-1.5">
                {columnMapping.textColumns.map(col => (
                  <span
                    key={col}
                    className="inline-flex px-2 py-0.5 rounded text-xs font-mono font-medium"
                    style={{ backgroundColor: 'rgba(22, 163, 74, 0.1)', color: '#166534' }}
                  >
                    {col}
                  </span>
                ))}
          ...
          {columnMapping.attachments.length > 0 && (
            <div className="flex items-start gap-3">
              <span className="text-xs font-medium w-20 flex-shrink-0 pt-0.5" style={{ color: colors.text.secondary }}>Attachments</span>
              <div className="flex flex-wrap gap-1.5">
                {columnMapping.attachments.map(a => (
                  <span
                    ...
                    style={{ backgroundColor: 'rgba(249, 115, 22, 0.1)', color: '#9a3412' }}
                  >
                    {a.column}
                    <span className="font-sans text-[10px] opacity-70">({a.type}, {a.format})</span>
                  </span>
                ))}
              </div>
            </div>
          ...
          {columnMapping.groundTruthColumns.length > 0 && (
            <div className="flex items-start gap-3">
              <span className="text-xs font-medium w-20 flex-shrink-0 pt-0.5" style={{ color: colors.text.secondary }}>Ground Truth</span>
              <div className="flex flex-wrap gap-1.5">
                {columnMapping.groundTruthColumns.map(col => (
                  <span
                    key={col}
                    className="inline-flex px-2 py-0.5 rounded text-xs font-mono font-medium"
                    style={{ backgroundColor: 'rgba(59, 130, 246, 0.1)', color: '#1e40af' }}
                  >
                    {col}
                  </span>
                ))}
      ...
      >
        <div className="pt-2">
          {promptTemplate ? (
            ...
            <div className="text-xs rounded-md p-3" style={{ backgroundColor: colors.bg.secondary, color: colors.text.secondary }}>
              No template set — all text columns will be concatenated by the backend.
            </div>
          )}
        </div>
      </AccordionSection>
          ...
          {configs.map((config, i) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <span className="text-sm font-medium" style={{ color: colors.text.primary }}>{config.name}</span>
                <span
      ...
      <AccordionSection
        ...
        onEdit={() => onEditStep(4)}
        badge={outputSchema.length > 0 ? `${outputSchema.length} properties` : 'None'}
      >
        <div className="pt-2">
          {outputSchema.length > 0 ? (
            ...
            <div className="text-xs rounded-md p-3" style={{ backgroundColor: colors.bg.secondary, color: colors.text.secondary }}>
              No schema defined — model will return free-form text.
            </div>
          )}
        </div>
      </AccordionSection>
      ...
      >
        ...
        <button
          ...
          style={{
            backgroundColor: isSubmitting ? colors.bg.secondary : colors.accent.primary,
