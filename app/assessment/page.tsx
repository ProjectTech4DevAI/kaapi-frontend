...
import PromptEditorStep from './components/PromptEditorStep';
import OutputSchemaStep from './components/OutputSchemaStep';
...
type TabId = 'datasets' | 'config' | 'evaluations';

const TABS: { id: TabId; label: string }[] = [
  { id: 'datasets', label: 'Datasets' },
  { id: 'config', label: 'Config' },
  { id: 'evaluations', label: 'Result' },
];
...
const CONFIG_STEPS: Step[] = [
  ...
  { id: 4, label: 'Output Schema' },
  { id: 5, label: 'Review' },
];

/* ── Shimmer dot indicator ── */
...
// Indicator state: which dot to show on the Evaluations tab
...
function AssessmentContent() {
  ...
  const [completedConfigSteps, setCompletedConfigSteps] = useState<Set<number>>(new Set());
  ...
  const dismissedRef = useRef(false); // tracks if user dismissed non-processing indicators
  const [assessmentRefreshToken, setAssessmentRefreshToken] = useState(0);

  // Form state
  const [experimentName, setExperimentName] = useState('');
  const { datasetId, columns, sampleRow, columnMapping, setDatasetId, setDataset, setColumnMapping } = useAssessmentDatasetStore();
  ...
  // Load assessment status for indicators
  const pollEvalStatus = useCallback(async () => {
    ...
    try {
      ...
      const hasProcessing = runs.some((r: { status: string }) =>
      ...
      // If user already dismissed, don't show failed/success
      if (dismissedRef.current) {
        setEvalIndicator('none');
        return;
      }

      // Check most recent run for failed/success (within last 5 minutes)
      if (runs.length > 0) {
        const recent = runs[0];
        ...
        if (updatedAt > fiveMinAgo) {
          if (recent.status === 'failed' || recent.status === 'completed_with_errors') {
  ...
  useAssessmentEvents(selectedKey?.key || '', () => {
    setAssessmentRefreshToken(prev => prev + 1);
    void pollEvalStatus();
  });

  // Dismiss non-processing indicators when switching to evaluations tab
  const handleTabSwitch = (tab: TabId) => {
    if (tab === 'evaluations') {
      if (evalIndicator === 'failed' || evalIndicator === 'success') {
  ...
  const handleColumnsLoaded = useCallback((cols: string[], firstRow: Record<string, string> = {}) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        attachments: columnMapping.attachments.map(({ column, type, format }) => ({ column, type, format })),
        output_schema: schemaToJsonSchema(outputSchema) || null,
        configs: configs.map(({ config_id, config_version }) => ({ config_id, config_version })),
      };
      ...
      // Switch to evaluations tab and refresh indicator
      dismissedRef.current = false;
  ...
  const indicatorStyles: Record<IndicatorState, { dot: string; underline: string }> = {
    none: {
      dot: 'transparent',
    ...
    <div className="w-full h-screen flex flex-col" style={{ backgroundColor: colors.bg.secondary }}>
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {/* Header */}
          <div className="border-b px-4 py-3 flex items-center gap-3 flex-shrink-0" style={{ backgroundColor: colors.bg.primary, borderColor: colors.border }}>
            <button
            ...
            <div>
              <h1 className="text-base font-semibold" style={{ color: colors.text.primary, letterSpacing: '-0.01em' }}>
                Assessment
              </h1>
          ...
          {apiKeys.length === 0 ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-xs mb-4" style={{ color: colors.text.secondary }}>Add an API key in the Keystore first</p>
            ...
            <>
              {/* Tab bar */}
              <div className="flex-shrink-0 border-b flex" style={{ backgroundColor: colors.bg.primary, borderColor: colors.border }}>
                {TABS.map(tab => {
                  const isActive = activeTab === tab.id;
                  const showIndicator = tab.id === 'evaluations' && evalIndicator !== 'none';
                  ...
                  return (
                    ...
                    >
                      {tab.label}
                      {showIndicator && <ShimmerDot color={indicatorColor[evalIndicator]} />}
                      {isActive && (
                        <div
                ...
                <div className="flex-1 overflow-hidden flex flex-col">
                  {!hasDataset ? (
                    <div className="flex-1 flex items-center justify-center">
                      <div className="text-center">
                        <svg className="mx-auto h-12 w-12 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: colors.border }}>
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 7v10c0 2 3.6 3 8 3s8-1 8-3V7M4 7c0 2 3.6 3 8 3s8-1 8-3M4 7c0-2 3.6-3 8-3s8 1 8 3" />
                        </svg>
                        <p className="text-sm font-medium mb-1" style={{ color: colors.text.primary }}>No dataset selected</p>
                        <p className="text-xs mb-4" style={{ color: colors.text.secondary }}>Select a dataset first from the Datasets tab</p>
                        <button
