...
import type { ModelPanelProps } from "@/app/lib/types/assessment";
import type { CompletionConfig } from "@/app/lib/types/configs";
import ConfigParamControl from "../prompt-config/ConfigParamControl";

const selectClass =
  "w-full rounded-full border border-border bg-bg-primary px-2 py-2 text-sm text-text-primary outline-none focus:ring-1";

// Provider + model pickers with the model's params behind an Advanced fold.
// Shared by the Pre-filter and Assessment sections.
      ...
      {Object.keys(paramDefs).length > 0 && (
        <details className="rounded-xl border border-border bg-bg-secondary">
          ...
          <div className="px-4 pb-4">
            {Object.entries(paramDefs).map(([paramKey, definition]) => (
              <div
                key={paramKey}
              ...
              >
                ...
                <ConfigParamControl
                  value={params[paramKey] ?? definition.default}
