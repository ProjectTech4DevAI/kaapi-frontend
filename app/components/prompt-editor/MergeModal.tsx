      ...
      >
        ...
        <div className="mb-4">
          ...
          >
            Merge{" "}
            <strong
              style={{ color: getUnifiedBranchColor(commits, currentBranch) }}
            >
              {currentBranch}
            </strong>{" "}
            into:
        ...
        {mergeToBranch &&
          (() => {
            const lastMerge = commits
              .filter(
                (c) =>
                  c.branch === mergeToBranch && c.mergeFrom === currentBranch,
              )
              .sort((a, b) => b.timestamp - a.timestamp)[0];
            const newCommits = lastMerge
              ? commits.filter(
                  (c) =>
                    c.branch === currentBranch &&
                    c.timestamp > lastMerge.timestamp,
                )
              : commits.filter((c) => c.branch === currentBranch);
            return (
              <div
                className="p-3 rounded-md mb-4 text-xs"
                style={{
                  backgroundColor:
                    newCommits.length > 0 ? "#dafbe1" : "#fff8c5",
                ...
                {newCommits.length > 0 ? (
                  <>
                    ...
                    <div style={{ color: "#116329", marginTop: "4px" }}>
                      {newCommits.length} new commit
                      {newCommits.length > 1 ? "s" : ""}
                    </div>
                  </>
                ) : (
                  <>
