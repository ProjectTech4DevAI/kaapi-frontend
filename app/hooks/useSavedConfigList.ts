...
interface UseSavedConfigListParams {
  configs: ConfigSelection[];
  setConfigs: StateSetter<ConfigSelection[]>;
}

// Saved-config listing + version-level selection, shared by the Experiment
// tab's picker and the config editor's save dialog.
export function useSavedConfigList({
  configs,
  setConfigs,
}: UseSavedConfigListParams): UseSavedConfigListResult {
