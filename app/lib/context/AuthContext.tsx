...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      if (stored) {
        ...
        setApiKeys(parsed);
        // Ensure cookie is in sync on hydration
        if (Array.isArray(parsed) && parsed.length > 0) {
          document.cookie = `kaapi_api_key=${encodeURIComponent(parsed[0].key)}; path=/; max-age=31536000; SameSite=Lax`;
        } else {
          document.cookie = "kaapi_api_key=; path=/; max-age=0; SameSite=Lax";
        }
      } else {
        document.cookie = "kaapi_api_key=; path=/; max-age=0; SameSite=Lax";
      }
  ...
  const persist = useCallback((keys: APIKey[]) => {
    ...
    if (keys.length > 0) {
      ...
      // Mirror the active key to a cookie so server components can read it
      document.cookie = `kaapi_api_key=${encodeURIComponent(keys[0].key)}; path=/; max-age=31536000; SameSite=Lax`;
    } else {
