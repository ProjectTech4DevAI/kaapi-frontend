...
### Post-review adjustments (same branch)

1. **Pre-filter mirrors the Assessment document** — Instructions (accept/reject criteria) + Submission (`@`-references) zones, sharing the same reference dataset and registering referenced columns into the same derived input schema. Since the backend has no pre-filter query template (§3.5), the Submission text is embedded into the sent `instructions` behind a deterministic `[Submission to judge]` marker (split back out on load) and also sent as a top-level `query_template` on `topic_relevance` for forward compatibility (`params` would reject it; top-level extras are ignored).
2. **Copy de-specialized** — "per submission row", "assess" instead of "grade" in all chrome copy; the teacher scenario survives only as the example content.
3. **Structured output is mandatory** — the free-text toggle is gone; a config cannot be saved without at least one named response field (blocking error in Review & save).
4. **New configs start prefilled with a short, real working example** (teacher grading a Class 8 Social Science paper from an id/name/submission dataset): real Instructions with a 4-question paper and marking rules, a Submission template using `{name}`/`{id}` with `submission` as a PDF attachment, the q1–q4 marks/reasoning/overall_feedback response fields, and matching pre-filter criteria prefilled on first enable. Placeholders remain as fallback teaching copy when an editor is emptied.
